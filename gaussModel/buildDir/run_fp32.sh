...
PX=`cat paramsDir/paramsCGFDM3D_fp32.json | grep "\"PX\"" | tr -cd "[0-9]"`
PY=`cat paramsDir/paramsCGFDM3D_fp32.json | grep "\"PY\"" | tr -cd "[0-9]"`
PZ=`cat paramsDir/paramsCGFDM3D_fp32.json | grep "\"PZ\"" | tr -cd "[0-9]"`

RUN=${MPIHOME}/bin/mpirun

${RUN} -np $(($PX*$PY*$PZ)) ./bin/CGFDM3D_fp32 | tee ./logDir/log_fp32
