...
void allocMomentRate( MOMENT_RATE * momentRate, long long pointNum, int nt )
{
	...
	checkCudaErrors( cudaMalloc( ( void ** )&pMomentRate, size ) );
	checkCudaErrors( cudaMemset( pMomentRate, 0, size ) );
...
void freeMomentRate( MOMENT_RATE momentRate, long long pointNum )
{
	...
	checkCudaErrors( cudaFree( momentRate.Mxx ) );
...
void allocSrcIndex( long long ** srcIndex, long long npts )
{
	...
	checkCudaErrors( cudaMalloc( (void **)srcIndex, size ) );
	checkCudaErrors( cudaMemset( *srcIndex, 0, size ) ); 
}
...
void freeSrcIndex( long long * srcIndex, long long npts )
{
	if ( npts == 0 )
		return;
	checkCudaErrors( cudaFree( srcIndex ) );
}
...
void freeMomentRateSlice( MOMENT_RATE momentRateSlice, long long npts  )
{
	if ( 0 == npts )
		return;
	checkCudaErrors( cudaFree( momentRateSlice.Mxx ) );
}
                       ...
                       MOMENT_RATE * momentRate, MOMENT_RATE * momentRateSlice, SOURCE_FILE_INPUT * ret_src_in )
{
	...
	if ( thisRank == 0 )
		printf( "Precessing fault(Source data precessing) data...\n"  );
	MPI_Barrier( MPI_COMM_WORLD );
	...
	checkCudaErrors( cudaMemcpy( momentRate->Mxx, momentRateNew.Mxx, mr_size, cudaMemcpyHostToDevice ));

	long long si_size = npts * sizeof( long long );
	checkCudaErrors( cudaMemcpy( *srcIndex, srcIndexNew, si_size, cudaMemcpyHostToDevice ));
...
__global__ void calculate_MomentRate( long long npts, long long nt, STRUCTURE structure_dev, float * Jac, MOMENT_RATE momentRate, long long * srcIndex, float DH )
{
	...
	if ( i >= 0 && i < npts && j >= 0 && j < nt )
	{
		...
		V = 1.0 / V;
		// V = V * Cv;
		momentRate.Mxx[pos] *= structure_dev.Vs[idx] * V;
		momentRate.Myy[pos] *= structure_dev.Vs[idx] * V;
		momentRate.Mzz[pos] *= structure_dev.Vs[idx] * V;
		momentRate.Mxy[pos] *= structure_dev.Vs[idx] * V;
		momentRate.Mxz[pos] *= structure_dev.Vs[idx] * V;
		momentRate.Myz[pos] *= structure_dev.Vs[idx] * V;
	}
...
void calculateMomentRate( SOURCE_FILE_INPUT src_in, STRUCTURE structure_dev, float * Jac, MOMENT_RATE momentRate, long long * srcIndex, float DH )
{
	...
	blocks.z = 1;
	calculate_MomentRate <<< blocks, threads >>>
	( npts, nt, structure_dev, Jac, momentRate, srcIndex, DH );

}


// get the maximum source value from multisource
void getMultiSourceMax( SOURCE_FILE_INPUT src_in, MOMENT_RATE momentRate, const float DT, float * source_max, float * es, const float sf )
{
	long long npts = src_in.npts;
	long long nt = src_in.nt;
	long long num = npts * nt * MOMENTSIZE;
	...
	if (npts != 0) {
		for (i = 0; i < numBlocks; i++)
		{
			...
			if ( i == numBlocks - 1 )
				cnt = retSize;
			checkCudaErrors( cublasIsamax(handle, cnt, momentRate.Mxx + stride, 1, &tmp_index) );
			max_index = stride + tmp_index - 1;
			checkCudaErrors( cudaMemcpy(&tmpMaxVl, &momentRate.Mxx[max_index], sizeof(float), cudaMemcpyDeviceToHost) );
	...
	MPI_Barrier( MPI_COMM_WORLD );
	MPI_Allreduce(&maxValue, source_max, 1, MPI_FLOAT, MPI_MAX, MPI_COMM_WORLD);
	*source_max = (*source_max);
	// *source_max = (*source_max) / 10;
	*es = -log2( sf * DT * (*source_max) );
	checkCudaErrors( cublasDestroy(handle) );

	printf("source_max = %f, es = %f\n", *source_max, *es);
	printf("new source = %f\n", pow(2.0f, *es) * DT * (*source_max));

}


__global__ void interp_momentRate( long long npts, 
...
__global__ void addSource( WAVE hW, MOMENT_RATE momentRateSlice, 
						   long long * srcIndex, int npts,
						   int gaussI, int gaussJ, int gaussK, float factorGauss,
						   int _nx_, int _ny_, int _nz_, int flagSurf, const float DT, const float cs )
{
	long long i = threadIdx.x + blockIdx.x * blockDim.x;
	...
	if ( i >= 0 && i < npts )
	{
		...
		//V = Jac[idx] * DH * DH * DH;
		//V = -1.0 / V;
		hW.Txx[idx] = hW.Txx[idx] - DT * cs * momentRateSlice.Mxx[i] * factorGauss;
		hW.Tyy[idx] = hW.Tyy[idx] - DT * cs * momentRateSlice.Myy[i] * factorGauss;
		hW.Tzz[idx] = hW.Tzz[idx] - DT * cs * momentRateSlice.Mzz[i] * factorGauss;
		hW.Txy[idx] = hW.Txy[idx] - DT * cs * momentRateSlice.Mxy[i] * factorGauss;
		hW.Txz[idx] = hW.Txz[idx] - DT * cs * momentRateSlice.Mxz[i] * factorGauss;
		hW.Tyz[idx] = hW.Tyz[idx] - DT * cs * momentRateSlice.Myz[i] * factorGauss;
	}
...
__global__ void addSource1( WAVE hW, MOMENT_RATE momentRateSlice, long long * srcIndex,
							int npts, const float DT, const float cs )
{
	long long i = threadIdx.x + blockIdx.x * blockDim.x;
	...
	if ( i >= 0 && i < npts )
	{
		...
		//V = Jac[idx] * DH * DH * DH;
		//V = -1.0 / V;
		hW.Txx[idx] = hW.Txx[idx] - DT * cs * momentRateSlice.Mxx[i];
		hW.Tyy[idx] = hW.Tyy[idx] - DT * cs * momentRateSlice.Myy[i];
		hW.Tzz[idx] = hW.Tzz[idx] - DT * cs * momentRateSlice.Mzz[i];
		hW.Txy[idx] = hW.Txy[idx] - DT * cs * momentRateSlice.Mxy[i];
		hW.Txz[idx] = hW.Txz[idx] - DT * cs * momentRateSlice.Mxz[i];
		hW.Tyz[idx] = hW.Tyz[idx] - DT * cs * momentRateSlice.Myz[i];
	}
...

{
	...
	checkCudaErrors( cudaDeviceSynchronize( ) );
#ifdef NO_SOURCE_SMOOTH
	addSource1  <<< blocks, threads >>> ( hW, momentRateSlice, srcIndex, npts, DT, cs );
	checkCudaErrors( cudaDeviceSynchronize( ) );
	...
	for( gaussK = - nGauss ; gaussK < nGauss + 1; gaussK ++ )
	{
		for( gaussJ = - nGauss; gaussJ < nGauss + 1; gaussJ ++ )
		{
			for( gaussI = - nGauss; gaussI < nGauss + 1; gaussI ++ )
			{
				...
				( hW, momentRateSlice, srcIndex, npts, gaussI, gaussJ, gaussK, _nx_, _ny_, _nz_, factorGauss, flagSurf, DT, cs );
				checkCudaErrors( cudaDeviceSynchronize( ) );
			}
		}
	}
