...
// Allocate device float memory for PML parematers
void allocPMLParameter( const GRID * const grid, PML_ALPHA * pml_alpha, PML_BETA *pml_beta, PML_D * pml_d )
{
	...
	checkCudaErrors( cudaMalloc((void **)&pParams, size) );
	checkCudaErrors( cudaMemset(pParams, 0, size) ); 
...
{
	checkCudaErrors( cudaFree(pml_alpha.x) );
}
...
void cpu_allocate_pml_para_memory( const GRID * const grid, PML_ALPHA * pml_alpha, PML_BETA * pml_beta, PML_D * pml_d )
{
	int _nx_ = grid->_nx_;
	int _ny_ = grid->_ny_;
	int _nz_ = grid->_nz_;
...
int cpu_init_pml_parameter( const GRID * const grid, const MPI_BORDER border, \
...
// Initialize PML parameters (pml_alpha, pml_beta, pml_d)
						 ...
						 const PML_ALPHA pml_alpha, const PML_BETA pml_beta, const PML_D pml_d )
{
	...
	long long size = sizeof( float ) * ( _nx_ + _ny_ + _nz_ ) * 3;
	checkCudaErrors( cudaMemcpy(pml_alpha.x, cpu_pml_alpha.x, size, cudaMemcpyHostToDevice) );
...
__global__ void PMLParameter_float2FLOAT( const PML_ALPHA_FLOAT pml_alpha_fp, const PML_ALPHA pml_alpha,
										  const long long num )
{
	...
	// if ( i >= 0 && i < _nx_ )
	// {
	// 	pml_alpha_fp.x[i] = __float2half(pml_alpha.x[i]);
	// 	pml_beta_fp.x[i]  = __float2half(pml_beta.x[i]);
	// 	pml_d_fp.x[i]     = __float2half(pml_d.x[i]);
	// }
	// else if ( i >= _nx_ && i < (_nx_ + _ny_) )
	// {
	// 	pml_alpha_fp.y[i] = __float2half(pml_alpha.y[i]);
	// 	pml_beta_fp.y[i]  = __float2half(pml_beta.y[i]);
	// 	pml_d_fp.y[i]     = __float2half(pml_d.y[i]);
	// }
	// else if ( i >= (_nx_ + _ny_) && i < num )
	// {
	// 	pml_alpha_fp.z[i] = __float2half(pml_alpha.z[i]);
	// 	pml_beta_fp.z[i]  = __float2half(pml_beta.z[i]);
	// 	pml_d_fp.z[i]     = __float2half(pml_d.z[i]);
	// }
	if ( i >= 0 && i < num )
	{
		pml_alpha_fp.x[i] = __float2half(pml_alpha.x[i]);
...
__global__ void residual( const PML_ALPHA_FLOAT pml_alpha_fp, const PML_ALPHA pml_alpha, const long long num )
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;

	if ( i >= 0 && i < num)
	{
		float res = float(pml_alpha.x[i] - pml_alpha_fp.x[i]);
		printf("%f\t", res );
	}
}


// Convert (pml_alpha, pml_beta, pml_d) float to half
							  ...
							  const PML_D_FLOAT pml_d_fp, const PML_D pml_d )
{
	...
	int _ny_ = grid->_ny_;
	int _nz_ = grid->_nz_;
	long long num = ( _nx_ + _ny_ + _nz_ ) * 3;
	...
	blockX.z = 1;
// printf("%s: %d\n", __FILE__, __LINE__);
	PMLParameter_float2FLOAT <<< blockX, threadX >>>
	( pml_alpha_fp, pml_alpha, num );

	// residual <<< (num + 512 - 1) / 512, 512 >>> (pml_alpha_fp, pml_alpha, num);
	// checkCudaErrors( cudaDeviceSynchronize() );
}